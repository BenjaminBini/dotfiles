...
(function () {
  UC.styloaix = {
    exec: function (win) {
      if (
        win.location.href !== _uc.BROWSERCHROME &&
        win.location.href !== this.TBDETACHEDMAIL &&
        win.location.href !== this.TBMAILCOMPOSEWIN
      )
        return;
      ...
      if (AppConstants.MOZ_APP_NAME !== "thunderbird") {
        ...
        if (win.location.href === _uc.BROWSERCHROME) {
          ...
          if (xPref.get(this.PREF_FIRSTRUN)) {
            ...
            if (
              !toolbar.getAttribute("currentset").split(",").includes(btn.id)
            ) {
    ...
    init: function () {
      ...
      this.prefListenerDisabledStyles = xPref.addListener(
        this.PREF_STYLESDISABLED,
        (sDisabled) => {
          let newSet;
          try {
            ...
            xPref.set(
              this.PREF_STYLESDISABLED,
              JSON.stringify([...this.disabledStyles])
            );
            return;
          }
      ...
      this.prefListenerStateEnabled = xPref.addListener(
        this.PREF_DISABLED,
        (disabled) => {
          ...
          _uc.windows((doc) => {
            ...
            doc
              .getElementById("styloaix-button")
              .classList.replace(...this.btnClasses);
            doc.getElementById("styloaix-reload-all").disabled = disabled;
          });
        }
      );

      this.prefListenerTbBtnRef = xPref.addListener(
        this.PREF_TBBTNREFELEMENT,
        () => {
          this.insertTbButton();
        }
      );

      this.prefListenerTbBtnRefPos = xPref.addListener(
        this.PREF_TBBTNREFPOSITION,
        () => {
          this.insertTbButton();
        }
      );
      ...
      this.disabledStyles = new DisabledSet(
        JSON.parse(xPref.get(this.PREF_STYLESDISABLED)).sort((a, b) =>
      ...
      if (!this.enabled) this.btnClasses.reverse();
    ...
    insertTbButton(btn) {
      ...
      let refElement = win.document.querySelector(
        xPref.get(this.PREF_TBBTNREFELEMENT)
      );
    ...
    createButton(doc) {
      let btn = _uc.createElement(doc, "toolbarbutton", {
        ...
        class:
          ...
          this.btnClasses[1],
        onclick:
          "if (event.button === 1 && event.target.id == this.id) UC.styloaix.enabled = !UC.styloaix.enabled;",
      });
      ...
      let newStylePopup = _uc.createElement(doc, "menupopup", {
        id: "styloaix-newstyle-popup",
      });

      let newPageStyleBtn = _uc.createElement(doc, "menuitem", {
        label: "For this page",
        oncommand:
          'UC.styloaix.openEditor({url: gBrowser.currentURI.specIgnoringRef, type: "url"});',
      });
      ...
      let newSiteStyleBtn = _uc.createElement(doc, "menuitem", {
        label: "For this site",
        oncommand:
          'let host = gBrowser.currentURI.asciiHost; UC.styloaix.openEditor({url: host || gBrowser.currentURI.specIgnoringRef, type: host ? "domain" : "url"});',
      });
    ...
    loadStyles: function () {
      let files = this.CSSDIR.directoryEntries.QueryInterface(
      ...
      if (this.enabled) this.toggleAll({ disable: false });
    },
    ...
    toggleStyle: function (
      style,
      { aStatus = !style.enabled, changeStatus = true, forced = false } = {}
    ) {
      if (this.enabled || forced) {
        if (aStatus) style.register();
        else style.unregister();
      }
      if (changeStatus) {
    ...
    forceRefresh: function () {
      ...
      let oppositeColorScheme = Services.appinfo.chromeColorSchemeIsDark
        ? "light"
        : "dark";
      let mql = win.matchMedia(
        "(prefers-color-scheme: " + oppositeColorScheme + ")"
      );
      let lis = function () {
    ...
    changeStatus: function (style, aStatus) {
      ...
      _uc.windows((doc) => {
        let menuitem = doc
          .getElementById("styloaix-popup")
          .getElementsByAttribute("styleid", style.fullName)[0];
    ...
    addStyleInMenu(style, popup) {
      let menuitem = _uc.createElement(popup.ownerDocument, "menuitem", {
        ...
        class:
          "styloaix-style" +
          (style.type == _uc.sss.AUTHOR_SHEET
            ? ""
            : " styloaix-" +
              (style.type == _uc.sss.USER_SHEET ? "user" : "agent") +
              "sheet"),
        acceltext:
          style.type == _uc.sss.AGENT_SHEET
            ? "AG"
            : style.type == _uc.sss.USER_SHEET
            ? "US"
            : "",
    ...
    populateMenu(e) {
      ...
      if (popup.id !== "styloaix-popup") return;

      const doc = e.view.document;

      const stylesSeparator = popup.querySelector("#styloaix-separator");
      while (stylesSeparator.nextSibling) stylesSeparator.nextSibling.remove();

      const sortedMap = new Map(
        [...UC.styloaix.styles.entries()].sort((a, b) =>
          a[0].localeCompare(b[0])
        )
      );
    ...
    openEditor({ id, url, type } = {}) {
      if (xPref.get(this.PREF_OPENINWINDOW)) {
        Services.wm
          ...
          .openDialog(
            this.EDITOR_URI,
            (id || Math.random()) + " - StyloaiX Editor",
            "centerscreen,chrome,resizable,dialog=no",
            { id, url, type }
          );
      } else {
        let appendUrl = "";
        if (id != undefined) appendUrl = "?id=" + id;
        else if (url)
          appendUrl = "?url=" + encodeURIComponent(url) + "&type=" + type;
        Services.wm
          .getMostRecentBrowserWindow()
          .switchToTabHavingURI(this.EDITOR_URI + appendUrl, true);
      }
    },
    ...
    get CSSDIR() {
      ...
      if (!cssFolder.exists() || !cssFolder.isDirectory())
        cssFolder.create(
          ...
          0o755 /*FileUtils.PERMS_DIRECTORY*/
        );
    ...
    STYLE: {
      get url() {
        return (this.url = Services.io.newURI(
    ...
    destroy: function () {
      ...
      if (
        Services.wm.getMostRecentWindow(null).AppConstants.MOZ_APP_NAME !==
        "thunderbird"
      )
        Services.wm
          .getMostRecentBrowserWindow()
          .CustomizableUI.destroyWidget("styloaix-button");
      else this.tbButtons.forEach((b) => b.remove());
    ...
    delete() {
      if (
        Services.prompt.confirm(
          null,
          "StyloaiX",
          'Delete "' + this.fullName + '" permanently?'
        )
      ) {
        UC.styloaix.styles.delete(this.fullName);
        if (this.enabled) this.unregister();
        else UC.styloaix.disabledStyles.delete(this.fullName);
