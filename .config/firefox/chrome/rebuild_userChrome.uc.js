...
UC.rebuild = {
  ...
  onHamPopup: function (aEvent) {
    ...
    Object.values(_uc.scripts)
      ...
      .forEach((script) => {
        ...
        scriptMenuItem.addEventListener("click", (event) =>
  ...
  elBuilder: function (doc, tag, props) {
    ...
    for (let p in props) {
      let val = props[p];
      if (typeof val == "function") el.addEventListener(p, val);
  ...
  createButton(aDocument) {
    ...
    let miToolbox = UC.rebuild.elBuilder(aDocument, "menuitem", {
      id: "menuitem_toolbox",
      label: "Open Browser Toolbox",
      class: "menuitem",
      command: function () {
        // Open Browser Toolbox via Services.obs
        try {
          const lazy = {};
    ...
    let mi2 = UC.rebuild.elBuilder(aDocument, "menu", {
      id: "showToolsMenu",
  ...
  createPanel(aDocument) {
    ...
    if (viewCache) {
      ...
      scriptsButton.addEventListener("command", function () {
        aDocument.ownerGlobal.PanelUI.showSubView(
          "appMenu-userChromeJsView",
          this,
        );
      });
